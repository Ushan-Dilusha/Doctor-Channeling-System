<?php
/*
    Signs out the user from a doctor account.
*/

//Checks whther the doctor_id is set in $_SESSION superglobal variable.
    //if not set, then seesion_start is used to resume a prevous session (if the relevant cookie exists) otherwise a new session is created
    if(!isset($_SESSION['doctor_id'])){
        session_start();
    }
    //again checks whether doctor_id is set. If not, it means the user is not logged in, and redirects the user to sign-in page.
    if(!isset($_SESSION['doctor_id'])){
    ...
    //redirects user to doctor sign in page.
    header('Location: .\signInDoctors.php');
    exit();
?>